...
function PokemonCacheProvider({children, cacheTime}) {
  ...
  const expiration = React.useRef({})

  React.useEffect(() => {
    const intervalId = setInterval(() => {
      // for(const [name, time] of Object.entries(expiration.current)) {
      //   if(time < Date.now()) {
      //     delete cache.current[name]
      //     // delete expiration.current[name]
      //   }
      // }
      const keys = Object.keys(expiration.current)
      for(const key of keys) {
        const prop = expiration.current[key]
        if(expiration.current[prop] < Date.now()) {
          delete cache.current[prop]
          delete expiration.current[prop]
        }
      }
    }, 1000)
    return () => {
  ...
  const getPokemonResource = React.useCallback(name => {
    ...
    let resource = cache.current[smallcaseName]
    if(!resource) {
      resource = createPokemonResource(smallcaseName)
      cache.current[smallcaseName] = resource
    }
    expiration[smallcaseName] = Date.now() + cacheTime
    return resource
  }, [cacheTime])
  return (
