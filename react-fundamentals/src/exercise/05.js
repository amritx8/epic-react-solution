...
function Box({size = '', color = '', ...otherProps}) {

  const className = 'box ' + (size ? `box--${size}` : '');
  const style = {
    fontStyle: 'italic',
    backgroundColor: color
  }

  return <div className={className.trim()} style={style} {...otherProps}>{otherProps.children}</div>
}
